...
namespace OpenDKPShared.Operations
{
    ...
    public static class RaidOperations
    {
        ...
        /// <returns></returns>
        public static IEnumerable GetCharacterRaids(
            string pClientId,
        ...
        )
        {
            ...
            {
                var vCharacter = vDatabase.Characters.First(x =>
                    x.ClientId.Equals(pClientId)
                    && x.Name.Equals(pCharacterName, StringComparison.InvariantCultureIgnoreCase)
                );
                List<TicksXCharacters> vResult = null;
                if (pLookback > 0)
                {
                    vResult = vDatabase
                        .TicksXCharacters.Include("IdTickNavigation.Raid.IdPoolNavigation")
                        .Where(x =>
                            x.ClientId.Equals(pClientId)
                            && x.IdCharacter == vCharacter.IdCharacter
                            && DateTime.Compare(
                                x.IdTickNavigation.Raid.Timestamp.Date,
                                DateTime.Now.AddDays(pLookback * -1).Date
                            ) >= 0
                        )
                        .ToList();
                }
                ...
                {
                    vResult = vDatabase
                        .TicksXCharacters.Where(x =>
                            x.ClientId.Equals(pClientId) && x.IdCharacter == vCharacter.IdCharacter
                        )
                ...
                foreach (var vModel in vResult)
                {
                    ...
                    if (!vRaids.ContainsKey(vModel.IdTickNavigation.RaidId))
                    {
                        ...
                        vRaid.TotalTicks = vDatabase
                            .Ticks.Include("Raid")
                            .Count(x => x.ClientId.Equals(pClientId) && x.RaidId == vRaid.IdRaid);
