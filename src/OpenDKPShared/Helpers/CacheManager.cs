...
namespace OpenDKPShared.Helpers
{
    public static class CacheManager
    {
        ...
        public static Cache GetCache(OpenDkpContext pContext, string pCacheName, string pClientId)
        {
            ...
            try
            {
                vReturnModel = pContext.Cache.FirstOrDefault(x =>
                    x.ClientId.Equals(pClientId)
                    && x.CacheName.Equals(pCacheName, StringComparison.InvariantCultureIgnoreCase)
                );
            }
        ...
        public static void SetCache(
            OpenDkpContext pContext,
            string pCacheName,
            string pCacheValue,
            string pClientId,
            DateTime? pCacheExpires = null
        )
        {
            ...
            try
            {
                Cache vCachedSummary = pContext.Cache.FirstOrDefault(x =>
                    x.ClientId.Equals(pClientId)
                    && x.CacheName.Equals(pCacheName, StringComparison.InvariantCultureIgnoreCase)
                );
                if (vCachedSummary == null)
                {
                    vCachedSummary = new Cache { CacheName = pCacheName, ClientId = pClientId };
        ...
        /// <returns></returns>
        public static PlayerSummaryModel GetPlayerSummary(
            OpenDkpContext pContext,
        ...
        public static async Task<int> UpdateItemCacheAsync(string pClientId)
        {
            ...
            // };
            // string vPayload = JsonConvert.SerializeObject(vProxyRequest);
            // InvokeResponse vResponse = await LambdaHelper.InvokeLambdaAsync(
            //     "GetAllItems",
            //     vPayload
            // );
            // return vResponse.StatusCode;
